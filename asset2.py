...
# Display Data
st.header("Fixed Assets Retirement Repository")
st.write("--------------------------------------", data)
...
st.write("Enter Asset Details for Retirement Readiness Analysis")
...
acquisition_date = st.date_input("Acquisition Date", value=datetime.date(2010, 1, 1))  # Allow dynamic date selection
...
# When the button is clicked, predict retirement readiness
if st.button('Predict Retirement Readiness'):
    ...
    retirement_status = "This Asset is Ready to Retire." if prediction[0] == 1 else "This Asset is not yet Ready to be Retired."
    st.write(f"Retirement Status: {retirement_status}")
